...
struct Individuo {
    ...
    void get_facility_LKM(int t) {
        ...
        for (int l = 0; l < L; l++) {
            ...
            bool ok = true;
            for (int st = t; st < nd; st++) {
                if ((int)open_facilities[st].size() == K) ok = false;
            }
            if (!ok) continue;
            for (int v = 0; v < V; v++) {
                for (int st = t; st < nd; st++) {
                    auto it = open_facilities[st].lower_bound({v, -1});
                    if (it != open_facilities[st].end() && it->first == v) {
                        ok = false;
                    }
                }
                if (!ok) continue;
        ...
        dna[t].insert({bv, bl});
        for (int st = t; st < std::min(T, t + center_types[bl]); st++) {
            open_facilities[st].insert({bv, t});
        }
    }
            ...
            if (rand() < K / (mean_l * V)) {
                for (int v = 0; v < V; v++) {
                    ...
                    if (ok) {
                        for (int st = t; st < nd; st++) {
                            auto it = open_facilities[st].lower_bound({v, -1});
                            while (it != open_facilities[st].end() && it->first == v) {
                                v = rand_i() % V;
                                it = open_facilities[st].lower_bound({v, -1});
                            }
                        }
                        for (int st = t; st < nd; st++) {
